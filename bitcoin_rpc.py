...
class BitcoinRPC(object):
    ...
    @defer.inlineCallbacks
    def getblock(self, hash, json='true'):
        connector, factory = self._call('getblock', [hash, json])
        resp = (yield factory.deferred)
        connector.transport = None
        defer.returnValue(json.loads(resp)['result'])

    @defer.inlineCallbacks
    def gettxout(self, txid, out_id_no=0):
        connector, factory = self._call('gettxout', [txid, out_id_no])
        resp = (yield factory.deferred)
        connector.transport = None
        defer.returnValue(json.loads(resp)['result'])
